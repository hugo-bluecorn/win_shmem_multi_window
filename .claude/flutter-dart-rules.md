# Flutter & Dart Development Rules for Claude Code

> **Source:** Adapted from [Flutter's AI Rules](https://github.com/flutter/flutter/blob/main/docs/rules/rules.md)
> **Purpose:** Guidelines for Claude Code when working with Flutter and Dart projects
...
## Effective Dart Rules - Complete Reference

The following rules are from the official [Effective Dart](https://dart.dev/effective-dart) guide. All rules should be followed when writing Dart code.
...
* **PREFER** starting comments with third-person verbs for side-effect focused functions (e.g., "Saves the document")
* **PREFER** starting non-boolean variable comments with a noun phrase (e.g., "The current user")
* **PREFER** starting boolean variable comments with "Whether" followed by a phrase (e.g., "Whether the user is logged in")
* **PREFER** noun phrases or non-imperative verbs for value-returning functions (e.g., "The distance" not "Gets the distance")
...
* **DON'T** use `.length` to see if a collection is empty (use `.isEmpty` or `.isNotEmpty`)
...
* **DON'T** use `this.` except to redirect or avoid shadowing
...
* **DO** test for `Future<T>` when disambiguating `FutureOr<T>`
...
* **PREFER** a non-imperative verb phrase for boolean properties (e.g., `isEnabled`, `canDelete`)
* **CONSIDER** omitting the verb for a named boolean parameter
* **PREFER** the "positive" name for a boolean property or variable
* **PREFER** an imperative verb phrase for side-effect focused functions (e.g., `save()`, `delete()`)
* **PREFER** noun phrases or non-imperative verbs for value-returning functions
* **CONSIDER** an imperative verb phrase to draw attention to work performed
* **AVOID** starting a method name with `get`
* **PREFER** naming a method `to___()` if it copies object state
* **PREFER** naming a method `as___()` if it returns a different representation
* **AVOID** describing parameters in the function or method's name
...
* **AVOID** defining a one-member abstract class when simple function will do
* **AVOID** defining a class with only static members
* **AVOID** extending a class not intended to be subclassed
* **DO** use class modifiers to control if your class can be extended
* **AVOID** implementing a class not intended to be an interface
* **DO** use class modifiers to control if class can be an interface
* **PREFER** defining pure `mixin` or `class` to a `mixin class`
...
* **AVOID** returning `this` from methods just to enable fluent interface

#### Types

* **DO** type annotate variables without initializers
* **DO** type annotate fields and top-level variables if type isn't obvious
...
* **DO** use `Future<void>` as return type for async members not producing values
* **AVOID** using `FutureOr<T>` as a return type

#### Parameters

* **AVOID** positional boolean parameters
* **AVOID** optional positional parameters if user may omit earlier parameters
* **AVOID** mandatory parameters that accept special "no argument" value
* **DO** use inclusive start and exclusive end parameters to accept a range

#### Equality

* **DO** override `hashCode` if you override `==`
* **DO** make your `==` operator obey mathematical rules of equality
...
* **Text Fields:** Configure `textCapitalization`, `keyboardType`, and input validators appropriately.
* **Error Handling for Remote Images:** When using network images, always provide an errorBuilder.
...
To use this guide with Claude Code:

1. **Reference in CLAUDE.md:** Add a reference to this file in your project's `CLAUDE.md`:
   ```markdown
   For Flutter/Dart projects, see [Flutter & Dart Rules](./flutter-dart-rules.md)
   ```

2. **Project-specific overrides:** If your project has specific requirements that differ from these guidelines, document them in your `CLAUDE.md` or a separate project-specific guidelines file.

3. **Keep updated:** As Flutter and Dart evolve, update this file to reflect new best practices.
